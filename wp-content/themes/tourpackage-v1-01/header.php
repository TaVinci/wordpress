					...
					<div class="top-navigation-wrapper boxed-style">
						<div class="top-navigation-container container">
							<?php
								...
								echo '<a href="./?page_id=122">
<span style="margin-left:  19px; font-size: 12px; line-height: 14px;  color: #000; background-color:#fff; padding: 2px 5px 2px 5px; -moz-border-radius: 3px; border-radius: 3px;">Blog</span></a>';	
...
window.onload = function(){ 
	...
	<!-- HTML for SEARCH BAR -->
	<div id="tfheader">
        <form method="get" id="tfnewsearch" action="<?php  echo home_url(); ?>/">

		        <input type="text" id="tfq" class="tftextinput2" name="s"  value="cauta ..."  autocomplete="off" data-default="<?php echo $search_val; ?>">
				...
				if( is_page() ){
					...
					if( $gdl_top_slider_type == 'Layer Slider' ){
						...
						//print_page_header(get_the_title(), $page_caption);
						 ...
						 /* //custom code subpages ©  paisionut.com */
                    	...
                    	<?php
 						...
 						if ( is_page() && $post->post_parent ) 
 						 ...
 						 $children = wp_list_pages("title_li=&child_of=".$post->ID."&echo=0&depth=1");
 						 //$children = preg_replace('%<li class=".*">|</?ul>%U', '', $children);
  						 //$children = str_replace('</li>', '', $children);
 						 $children = str_replace('Sejur Romania', '', $children);

  						 $children = str_replace(array('Charter','Sejur','Circuit','Croaziera', 'Last Minute'), '', $children);
...
<?php print_page_header(get_the_title() , '<div class="page-header-caption"> '. $children .' </div>' );?> 
						<?php } else print_page_header(get_the_title(), $page_caption); { ?>
