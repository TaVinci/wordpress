<?php
	...
	function print_package_item($item_xml){
		...
		$category = find_xml_value($item_xml, 'category', true);
		...
		//order post DESC by date to all package
		
		query_posts(array('post_type'=>'package', 'paged'=>$paged, 'order' => 'DESC', 'orderby'=>$orderby,
	...
	function print_single_package_thumbnail( $post_id, $item_size, $last_minute = 'normal-type', $last_text = 'Read More'  ){
		...
		if( !empty($thumbnail) ){
			...
			echo '<div class="port-media-wrapper gdl-image">';
				...
				echo '<a href="' . $thumbnail_full[0] . '" data-rel="fancybox" rel="grup" title="' . get_the_title() . '">';
				echo '<img src="' . $thumbnail[0] .'" alt="'. $alt_text .'"/>';
				echo '<div class="galerie">GALERIE FOTO</div>';
				echo '</a>';
		...
		function print_single_package_thumbnail2( $post_id, $item_size, $last_minute = 'normal-type', $last_text = 'Read More'  ){
		$thumbnail_id = get_post_meta( $post_id, 'post-option-inside-thumbnail2', true );
		$thumbnail_link = get_post_meta( $post_id, 'post-option-inside-thumbnail2-link', true );
		$thumbnail = wp_get_attachment_image_src( $thumbnail_id , $item_size );
		$thumbnail_full = wp_get_attachment_image_src( $thumbnail_id , 'full' );
		$alt_text = get_post_meta($thumbnail_id , '_wp_attachment_image_alt', true);
		if( !empty($thumbnail) ){
			echo '<div class="package-media-wrapper gdl-image" style="display:none;">';
			...
			function print_single_package_thumbnail3( $post_id, $item_size, $last_minute = 'normal-type', $last_text = 'Read More'  ){
		$thumbnail_id = get_post_meta( $post_id, 'post-option-inside-thumbnail3', true );
		$thumbnail_link = get_post_meta( $post_id, 'post-option-inside-thumbnail3-link', true );
		$thumbnail = wp_get_attachment_image_src( $thumbnail_id , $item_size );
		$thumbnail_full = wp_get_attachment_image_src( $thumbnail_id , 'full' );
		$alt_text = get_post_meta($thumbnail_id , '_wp_attachment_image_alt', true);
		if( !empty($thumbnail) ){
			...
			function print_single_package_thumbnail4( $post_id, $item_size, $last_minute = 'normal-type', $last_text = 'Read More'  ){
		$thumbnail_id = get_post_meta( $post_id, 'post-option-inside-thumbnail4', true );
		$thumbnail_link = get_post_meta( $post_id, 'post-option-inside-thumbnail4-link', true );
		$thumbnail = wp_get_attachment_image_src( $thumbnail_id , $item_size );
		$thumbnail_full = wp_get_attachment_image_src( $thumbnail_id , 'full' );
		$alt_text = get_post_meta($thumbnail_id , '_wp_attachment_image_alt', true);
		if( !empty($thumbnail) ){
			...
			echo '</div>';	// package-media-wrapper
				...
				function print_single_package_thumbnail5( $post_id, $item_size, $last_minute = 'normal-type', $last_text = 'Read More'  ){
		$thumbnail_id = get_post_meta( $post_id, 'post-option-inside-thumbnail5', true );
		$thumbnail_link = get_post_meta( $post_id, 'post-option-inside-thumbnail5-link', true );
		$thumbnail = wp_get_attachment_image_src( $thumbnail_id , $item_size );
		$thumbnail_full = wp_get_attachment_image_src( $thumbnail_id , 'full' );
		$alt_text = get_post_meta($thumbnail_id , '_wp_attachment_image_alt', true);
		if( !empty($thumbnail) ){
			...
			echo '</div>';	// package-media-wrapper
					...
					function print_single_package_thumbnail6( $post_id, $item_size, $last_minute = 'normal-type', $last_text = 'Read More'  ){
		$thumbnail_id = get_post_meta( $post_id, 'post-option-inside-thumbnail6', true );
		$thumbnail_link = get_post_meta( $post_id, 'post-option-inside-thumbnail6-link', true );
		$thumbnail = wp_get_attachment_image_src( $thumbnail_id , $item_size );
		$thumbnail_full = wp_get_attachment_image_src( $thumbnail_id , 'full' );
		$alt_text = get_post_meta($thumbnail_id , '_wp_attachment_image_alt', true);
		if( !empty($thumbnail) ){
			...
			echo '</div>';	// package-media-wrapper
					...
					function print_single_package_thumbnail7( $post_id, $item_size, $last_minute = 'normal-type', $last_text = 'Read More'  ){
		$thumbnail_id = get_post_meta( $post_id, 'post-option-inside-thumbnail7', true );
		$thumbnail_link = get_post_meta( $post_id, 'post-option-inside-thumbnail7-link', true );
		$thumbnail = wp_get_attachment_image_src( $thumbnail_id , $item_size );
		$thumbnail_full = wp_get_attachment_image_src( $thumbnail_id , 'full' );
		$alt_text = get_post_meta($thumbnail_id , '_wp_attachment_image_alt', true);
		if( !empty($thumbnail) ){
			...
			echo '</div>';	// package-media-wrapper
					...
					function print_single_package_thumbnail8( $post_id, $item_size, $last_minute = 'normal-type', $last_text = 'Read More'  ){
		$thumbnail_id = get_post_meta( $post_id, 'post-option-inside-thumbnail8', true );
		$thumbnail_link = get_post_meta( $post_id, 'post-option-inside-thumbnail8-link', true );
		$thumbnail = wp_get_attachment_image_src( $thumbnail_id , $item_size );
		$thumbnail_full = wp_get_attachment_image_src( $thumbnail_id , 'full' );
		$alt_text = get_post_meta($thumbnail_id , '_wp_attachment_image_alt', true);
		if( !empty($thumbnail) ){
			...
			echo '</div>';	// package-media-wrapper
					...
					function print_single_package_thumbnail9( $post_id, $item_size, $last_minute = 'normal-type', $last_text = 'Read More'  ){
		$thumbnail_id = get_post_meta( $post_id, 'post-option-inside-thumbnail9', true );
		$thumbnail_link = get_post_meta( $post_id, 'post-option-inside-thumbnail9-link', true );
		$thumbnail = wp_get_attachment_image_src( $thumbnail_id , $item_size );
		$thumbnail_full = wp_get_attachment_image_src( $thumbnail_id , 'full' );
		$alt_text = get_post_meta($thumbnail_id , '_wp_attachment_image_alt', true);
		if( !empty($thumbnail) ){
			...
			echo '</div>';	// package-media-wrapper
					...
					function print_single_package_thumbnail10( $post_id, $item_size, $last_minute = 'normal-type', $last_text = 'Read More'  ){
		$thumbnail_id = get_post_meta( $post_id, 'post-option-inside-thumbnail10', true );
		$thumbnail_link = get_post_meta( $post_id, 'post-option-inside-thumbnail10-link', true );
		$thumbnail = wp_get_attachment_image_src( $thumbnail_id , $item_size );
		$thumbnail_full = wp_get_attachment_image_src( $thumbnail_id , 'full' );
		$alt_text = get_post_meta($thumbnail_id , '_wp_attachment_image_alt', true);
		if( !empty($thumbnail) ){
	...
	function print_package_search_item($item_xml){
		...
		echo '<td><input type="text" name="s" id="package-search" value="' . $translator_key_words . '" data-default="' . $translator_key_words . '" /></td>';
		echo '</div>';
		...
		// make the categories and not the tags for the packages be what is used for the trip type area of the package search
		// paisionut.com (C)
		//$get_category = get_categories( array( 'taxonomy' => 'package-category', 'hide_empty' => 0	));
